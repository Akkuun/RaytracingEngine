...
FPSChart::FPSChart(QWidget *parent)
    : QWidget(parent), maxDataPoints(60), maxFPS(120)
{
...
void FPSChart::addFPSValue(int fps)
{
    ...
    // Update max FPS for scaling (with some headroom)
    if (fps > maxFPS * 0.9)
    {
        maxFPS = fps * 1.2;
    }
...
void FPSChart::paintEvent(QPaintEvent * /*event*/)
{
    ...
    for (int i = 0; i <= 4; i++)
    {
        ...
        // Draw FPS labels
        int fpsValue = maxFPS - (maxFPS * i) / 4;
        painter.setPen(QColor(136, 136, 136));
        painter.drawText(2, y + 5, QString::number(fpsValue));
        painter.setPen(QColor(60, 60, 60));
    }
